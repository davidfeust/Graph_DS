...
//        hodefTest();
    ...
    public static void hodefTest() {
        System.out.println("milion start");
        graph milion = new Graph_DS();
        for (int i = 0; i < 1000000; i++) {
            milion.addNode(new NodeData());
        }
        for (int i =10 ; i<milion.nodeSize(); i++){
            for (int j =0; j<10;j++){
                milion.connect(i,i-j);
            }
        }
        Graph_Algo m = new Graph_Algo(milion);

        System.out.println("shortest path: 10,30 dist="+ (m.shortestPathDist(10,150)));
        List<node_data> m1 = m.shortestPath(10,150);
